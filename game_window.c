...
void print_header(GameWindow* game_window){
	...
	attron(COLOR_PAIR(3));
	mvprintw(0, (game_window->width - 1) - (strlen("POINTS 185") + PADDING), "POINTS %d", game_window->usr_points);
	attroff(COLOR_PAIR(3));

	//Draw horizontal line
	move(1, 0);
	hline('='| COLOR_PAIR(3), game_window->width);
}

void print_footer(GameWindow* game_window){
	//Draw horizontal line
	move(game_window->height - 2, 0);
	hline('='| COLOR_PAIR(COLOR_GREEN), game_window->width);
